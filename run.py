...
if __name__ == "__main__":
    ...
    while True:
        ...
        boxes = []
        target_dim = [im_h*5/8, im_w*3/5]
        w_start = im_w*1/3-100
        w_end = im_w*2/3+300
        cv2.rectangle(im_out,(w_start , im_h-target_dim[0]),( w_end , im_h), (125,255,255), 3)
                ...
                for n in range(detectors):
                    ...
                    if np.sum(classes_scores * (classes_scores>.3)) > 0:
                        ...
                        if(CLASSES[box.get_label()] == "person"):
                            boxes += [box]
                            print(y)
                            print(im_h-y*im_h < target_dim[0])
                            print(im_h-y*im_h , target_dim[0])
                            if (im_h-y*im_h) < target_dim[0] and (x*im_w) > w_start and (x*im_w) < w_end :
                                should_break = True
        ...
        if(should_break):
            ...
            cv2.putText(im_out, "B R E A K",  (im_h/2+250, im_w/2),  cv2.FONT_HERSHEY_PLAIN, 3, (0, 0, 0), 6)
            cv2.putText(im_out, "B R E A K" , (im_h/2+250, im_w/2) , cv2.FONT_HERSHEY_PLAIN, 3, (255, 255, 255), 2)
        else :
